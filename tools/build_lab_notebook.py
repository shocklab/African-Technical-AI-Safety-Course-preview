...
M("### Explore ①",
  ...
  "- With `SUBTRACT_E = True` still set, change `E` to 1.5 or 1.9 and watch the fitted exponent move. With the flag off, `E` only moves the printed line and the fit does not budge. How confident can you be in a number that depends this much on a constant someone else estimated?",
...
C("CHINCHILLA = ('https://raw.githubusercontent.com/epoch-research/'",
  ...
  "    ok = target > 0",
  "    if ok.sum() < len(target):",
  "        print(f'note: {len(target) - ok.sum()} of {len(target)} frontier points dropped, so the fit uses {ok.sum()}')",
...
C("# Keep the rows that have both a date and a compute estimate.",
  "if models is None:",
  "    raise SystemExit('No Epoch data: fetch the CSV by hand, then re-run from here.')",
...
C("# run_with_cache captures every intermediate activation.",
  ...
  "print(f'tokens: {model.to_str_tokens(text, prepend_bos=False)}')",
  "print()",
...
C("# How the residual-stream norm moves through the blocks. Note the last block, which cuts it.",
  ...
  "ax.set_title('Residual-stream norm, block by block')",
  "plt.show()")
...
C("# Across many sentence pairs, not one: the ratio is the number that matters.",
  ...
  "    en_tok = sample['en'].apply(lambda s: len(model.to_str_tokens(s, prepend_bos=False)))",
  "    zu_tok = sample['zul'].apply(lambda s: len(model.to_str_tokens(s, prepend_bos=False)))",
...
C("# Optional: the same measurement across several African languages.",
  ...
  "        e = p['en'].apply(lambda s: len(model.to_str_tokens(s, prepend_bos=False))).mean()",
  "        o = p[col].apply(lambda s: len(model.to_str_tokens(s, prepend_bos=False))).mean()",
...
C("MY_PROMPTS = [",
  ...
  "    t = model.to_str_tokens(s, prepend_bos=False)",
